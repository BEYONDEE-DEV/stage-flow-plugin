...
class DocsSkillTests(unittest.TestCase):
    def test_skill_entry_routes_every_direct_reference(self) -> None:
        ...
        expected = {
            ...
            "shared-contract-readiness.md",
            "semantic-review-closure.md",
            "semantic-review-proof.md",
    ...
    def test_direct_references_separate_normative_owners_from_flow_consumers(self) -> None:
        ...
        unique_rules = {
            ...
            "normative owner of version-5 local/shared risk routing": "shared-contract-readiness.md",
            "normative owner of v5 embedded review receipts": "semantic-review-proof.md",
            "normative owner of the v5 request-local event journal": "operation-metrics.md",
    ...
    def test_storage_config_supports_repository_and_submodule(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "python3 <plugin-root>/scripts/validate_atomic_docs.py",
    ...
    def test_v5_aid_definitions_references_and_orphans_have_distinct_gates(self) -> None:
        text = refs(
            "atom-format-and-judgment.md",
            "docs-generation-flow.md",
            "validation-contract.md",
        ...
        assert_all(
            ...
            text,
            (
                "Treat `[AID:<value>]` only as the unique definition",
                "Consume that meaning everywhere else with exact `[AID-REF:<value>]`",
                "entire managed Atom set regardless of expected-key scope",
                "recognized request-local artifact",
                "Reject request-local definitions, every duplicate definition",
                "every indexed `[AID-REF:...]` with no definition immediately",
                "Ignore token-looking text in YAML frontmatter",
                "Parse CommonMark blocks/inlines with the plugin-vendored `markdown-it-py` AST",
                "fenced or indented code, inline code, HTML comments",
                "link/image destinations and titles",
                "Use the nearest list item as one consumer block",
                "each nested list item is distinct",
                "top-level `created_aids`",
                "unique wrapper-free AID values",
                "During intermediate bundle validation",
                "At the final request-bound selection/docs/baseline and terminal gates",
                "Do not apply this final orphan rule retroactively to existing AIDs",
                "Do not print AID totals in an ordinary PASS",
            ),
        )
    ...
    def test_general_atoms_skip_verification_targets_but_implementation_basis_keeps_them(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "changed in-scope required AID",
                "observable verification condition",
                "Cartesian test combinations",
                "Do not preserve Cartesian test combinations in managed docs",
    ...
    def test_high_risk_review_checks_selected_context_without_starting_an_audit(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "Record the risk ID, candidate ID, planned `atom_key`, trigger, selected contract basis, route, and trace before review",
                ...
                "correct state, rerun version-5 selection preflight",
                "rerun version-5 selection preflight before the writer or semantic rerun",
            ),
        )
    ...
    def test_shared_owner_prepass_orders_queue_without_freezing_all_owners(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "For multi-domain or `initial-baseline` work, run one prepass",
                "permission`, `shared-identifier`, `money-entitlement`, `final-projection`, or `integration",
                "Confirm shared owners whose later change would reopen direct consumers",
                "keep ordinary single-domain owners provisional",
                "owner bundle precedes every direct consumer bundle",
                "Do not build a complete semantic graph",
    ...
    def test_version_five_shared_contract_reference_owns_bounded_state(self) -> None:
        ...
        assert_all(
            ...
            skill + readiness,
            (
                "selection contract version 5",
                "context_selection.version: \"5\"",
                "route: local|shared-contract",
                "shared_contract_id",
                "shared_contracts",
                "selection_readiness",
                ...
                "reviewer_handoffs",
                "risk_id",
                'selection_retirements: {"version":"1","retired_bundles":[],"retired_contracts":[]}',
                "bundle_id",
        ...
        self.assertIn(
            "normative version-5 local/shared risk routes",
            skill,
        )

    def test_version_five_closes_only_high_fan_out_contracts(self) -> None:
        text = refs("shared-contract-readiness.md", "service-logic-coverage.md")
        assert_all(
            self,
            text,
            (
                "High-fan-out means the contract has an owner and at least one direct consumer in a different bundle",
                "Do not promote every risk candidate or every aggregate owner",
                "owner_candidate_id",
                "owner_atom_key",
                ...
                "consumer_bundle_ids",
                "owner bundle precedes every direct consumer bundle",
                "Every declared direct consumer has a candidate-linked shared route",
                "transaction order, retry details, projection fields",
            ),
        )

    def test_version_five_retirement_history_preserves_removed_identities(self) -> None:
        text = refs(
            "shared-contract-readiness.md",
            "docs-generation-flow.md",
            "semantic-review-closure.md",
        ...
        assert_all(
            ...
            text,
            (
                "`retired_bundles` row copies the active queue entry's",
                "`retired_contracts` row copies all eight active contract fields",
                "`retired_basis_revision`",
                "Retirement rows are immutable append-only history",
                "ordered by nondecreasing `retired_basis_revision` no later than the current readiness basis",
                "snapshot readiness basis < retired_basis_revision <= current readiness basis",
                "Active and retired IDs are disjoint",
                "exact snapshot-active bundle or contract",
                "Current risk routing, binding traces, dependencies, owner/consumer closure, queue order, and a new/open discovery resolve active IDs only",
                "Historical version-5 development/risk scopes",
                "active plus retired history",
                "may remove only a dependency or `shared-contract` risk route that directly names the newly retired contract",
                "participant's local route or route to another contract remains exact",
                "Couple retirement to the pre-mutation snapshot without broadening impact",
                "every prior `current|stale` development/risk PASS scoped to its stable bundle ID",
                "snapshot must show each retirement-affected PASS as `stale`",
                "`required_reviews` contains the exact `(reviewer_role, bundle_id)` pair for every stale PASS",
                "every pre-retirement development and risk PASS in that stable scope is retirement-affected and must become stale",
                "no current pre-retirement risk PASS in that scope is reusable",
                "Risk-neutral reuse applies only outside the retirement-affected stable scope",
                "non-retirement correction whose risk basis is independently unchanged",
                "`opened_revision == retired_basis_revision`",
                "If the bundle has no prior semantic PASS, retirement needs no invalidation",
                "every owner/consumer stable bundle scope still live at retirement necessarily changes its contract route, dependency, or identity basis",
                "live scope without a prior PASS needs no invalidation",
                "related contract retired by one correction share one `retired_basis_revision`",
                "Every current version-5 selection also revalidates the entire retirement history",
                "not only newly appended snapshot transitions",
                "basis_revision < T",
                "resolved_revision < T",
                "basis_revision >= T",
                ...
                "reject a missing, non-string, v1-v4, future, or unknown marker",
                "Never mutate, resume, migrate, backfill, or dual-read rejected state",
            ),
        )

    def test_version_five_reuses_development_reviewer_before_writer(self) -> None:
        text = refs(
            "shared-contract-readiness.md",
            "docs-generation-flow.md",
            "reviewer-perspectives.md",
        ...
        assert_all(
            ...
            text,
            (
                "same persistent reviewer first performs the selection-readiness review",
                "not a fourth reviewer role",
                "persistent_agent_ids.reviewer",
                ...
                "After Goal handoff, every executable Atomic Docs operation uses exact `context_selection.version: \"5\"`",
            ),
        )

    def test_v5_receipts_challenge_and_handoff_preserve_basis_identity(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                '"report_path"',
                '"report_sha256"',
                "order `docs` by `path` and `source_locators` by `locator`, and reject duplicates",
                "sorted object keys, compact separators `,` and `:`, and direct non-ASCII code points (`ensure_ascii=false`)",
                "project-root-relative, symlink-free at every lexical component",
                "configured docs root's project-root-relative `project/atomization-criteria.md`",
                "HEAD-reachable ancestor `source_commit_observed`",
                "Resolve every source locator lexically from the repository tree at that revision",
                "historical `stale|superseded` readiness review instead matches the persistent reviewer that was active at its basis",
                "do not compare that historical docs digest with mutable current managed-doc bytes",
                "mode: reuse-final-review|dedicated",
                "exactly `primary-report`, `primary-verdict`, and `primary-evidence-summary`",
                "raw `inventory.md`, raw `evidence.md`",
                "every current managed Atom required by the terminal scope",
                "Do not hash `work-state.json` or invent a receipt-excluded state projection",
                "receipt role `challenger` and scope `terminal-current-basis`",
                "journal-projected `integration-review` span",
                "`span_id` equals the challenge `review_id`",
                "Run at most one challenge attempt for a basis revision",
                "Final closure requires exactly one attempt at the current closure basis and exact `PASS`",
                "Treat a material challenge FAIL as new semantic evidence",
                "increment the basis exactly once, open the affected invalidation",
                "hand off the primary with reason `challenger-material-fail`",
                "At a handoff basis, `to_agent_id` becomes the basis-active primary until the next handoff",
                "Set only the latest `to_agent_id` as the current `persistent_agent_ids.reviewer`",
                "`context-saturation`, `agent-unavailable`, `basis-reset`, or `challenger-material-fail`",
                "`context_check_review_id`",
                "fourth distinct completed queue item",
                "replacement's first readiness/development action must start later in journal sequence",
                "After the fourth completed bundle",
            ),
        )

    def test_v5_binding_trace_review_promotes_without_pre_writer_cycle(self) -> None:
        text = refs("shared-contract-readiness.md", "validation-contract.md")
        assert_all(
            self,
            text,
            (
                "During pre-writer or intermediate selection",
                "current receipt-bound `selection_readiness` PASS at the trace basis",
                "avoids requiring a semantic bundle PASS before any writer can run",
                "promote the trace to that current receipt-bound `semantic_review_closure.review_passes` PASS",
                "Final selection with `--require-actions-final`",
                "reject a readiness-only trace",
                "pre-writer/intermediate traces may bind current same-basis readiness",
                "final traces bind current receipt-bound semantic closure",
            ),
        )

    def test_v5_binding_trace_schema_covers_contract_and_permission_resources(self) -> None:
        readiness = read(REFS / "shared-contract-readiness.md")
        assert_all(
            self,
            readiness,
            (
                "exactly one trace for every selected `risk_triggers.risk_id`",
                ...
                "observed_conflict",
                "kind: contract|permission-privacy",
                "authority_status: verified|unresolved",
                "applicability_status: verified|unresolved",
                "principal`, `authorized_object_or_data`, `loaded_object`, and `projection_or_use`",
                "never normalize cross-resource sensitive-data exposure into a healthy Rule",
                "only when `observed_conflict: true` and both authority and target applicability are `verified`",
                "Verified status alone does not prove a conflict",
            ),
        )

    def test_reviewer_stop_rule_escalates_only_bounded_shared_contracts(self) -> None:
        text = refs("reviewer-perspectives.md", "shared-contract-readiness.md")
        assert_all(
            self,
            text,
            (
                "Stop completeness discovery once selected claims can be judged",
                "directly evidenced omitted bounded high-fan-out contract",
                "ordinary API, branch, transaction, projection, failure, or attack-path detail cannot expand",
                "apply `drop`, `remove`, or `generalize`",
                "does not claim every source behavior, API, aggregate, or possible consumer was inventoried",
            ),
        )

    def test_late_discovery_and_same_root_failures_pause_only_affected_dispatch(self) -> None:
        readiness = read(REFS / "shared-contract-readiness.md")
        ...
        assert_all(
            ...
            readiness + closure,
            (
                "stop new bundle dispatch",
                "Update the bounded prepass, evidence routes, risk routes, dependencies, and queue order",
                "Stale and rerun only the owner and direct consumer bundles whose recorded basis changed",
                "carry unrelated PASSes forward",
                "candidate-admission",
                "contract-routing",
                "owner-evidence",
                "consumer-closure",
                ...
                "consecutive first-attempt FAIL entries share the same category and at least one candidate or contract root",
                "diagnosis",
                "resume_basis_revision",
                "semantic_invalidation_ids: []",
                "no affected bundle has a semantic PASS yet",
                "strictly greater than every trigger diagnostic basis",
                "do not start a new `bundle` or `writer` dispatch",
                "exactly one matching `late-shared-contract` dispatch episode",
                "immutable `pause_after_span_id`",
                "positive `pause_after_sequence`",
                "RFC3339 `paused_at`",
                "sequence equals that span's `finished_sequence`",
                "journal sequence is the causal cutoff",
                "whose `started_sequence` is after the cutoff",
                "timestamps or list placement suggest otherwise",
                "does not add these late-contract fields",
                "starts strictly after the cutoff",
                "raw FAIL count",
                "elapsed time",
                "leave unrelated bundle PASSes current",
            ),
        )

    def test_v5_validator_boundary_is_structural_not_semantic(self) -> None:
        text = refs("shared-contract-readiness.md", "validation-contract.md")
        assert_all(
            self,
            text,
            (
                "reviewer identity reuse",
                "owner-before-consumer ordering",
                "whether a source concern is truly local",
                "every real consumer was found",
                "authority/applicability is semantically valid",
                "two FAILs share one root cause",
                "A structural PASS never replaces required subagent review",
                "Before request-bound final docs/baseline validation, rerun full selection with `--require-actions-final`",
                "both `metrics-preterminal` and `metrics-final` checks",
                "Keep request-local inventory, evidence, receipts, and findings-only reports through these checks",
                "immutable retirement history and historical references",
                "each stale development/risk PASS ID requires its exact `(reviewer_role, stable bundle_id scope)` pair",
                "including late-discovery and general-correction invalidations",
                "A current risk PASS not listed stale adds no pair",
                "rejects every other post-Goal marker and never migrates it",
                "journal-projected pause/resume ordering",
                "Selection validation applies only to exact version 5 after Goal handoff",
                "receipt proves honest observation",
            ),
        )
    ...
    def test_operation_evidence_index_is_reused_but_not_trusted_as_proof(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "smallest authoritative source locator",
                "one concise relevance statement",
                "## <candidate_id>",
    ...
    def test_domain_bundles_reuse_persistent_writer_and_reviewers(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "Create one independent development-quality reviewer as the persistent primary",
                "After its receipt-bound PASS, create one writer",
                "record its agent ID",
                "record its ID",
                "Reuse both across sequential bundles",
                "create one risk/contract reviewer distinct from writer and primary",
                "reuse it for later triggered bundles",
                "Replace the primary only through `semantic-review-proof.md`",
                "The writer receives only active-bundle `write` candidates",
    ...
    def test_bundle_preflight_precedes_parallel_semantic_review(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "Build the queue only from `write` keys and targets consumed by `merge`",
                "repair operation state, inventory, evidence locator/digest/question, queue, readiness receipt, risk reference, or binding trace",
                "--expect-atom-key <key>",
    ...
    def test_post_pass_semantic_changes_open_and_resolve_affected_closure(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "Every post-Goal operation uses exact `context_selection.version: \"5\"`",
                ...
                "Review PASS and invalidation records are append-only history",
                "`review_history` is append-only",
                "Once an invalidation is resolved, leave it unchanged",
                "expand that open invalidation before editing",
                "Do not create overlapping invalidations that name the same stale PASS",
                "current-operation Atom drop remains automatic",
                "exact approved existing-Atom action keeps its prior authorization",
                "Resolve an invalidation only when every required pair",
                "change its listed old PASSes from `stale` to `superseded`",
                "run the validator with the request ID",
            ),
        )

    def test_final_reviewer_reconciles_current_corpus_and_canonical_decisions(self) -> None:
        reviewers = read(REFS / "reviewer-perspectives.md")
        project_docs = read(REFS / "project-documents-and-inventory.md")
        assert_all(
            self,
            reviewers + project_docs,
            (
                "Independently inspect the current managed docs and operation inventory",
                "glossary source-of-truth terms with domain context and Atom ownership",
                "canonical root decisions/Gaps with consumer references",
                "Upstream PASS reports and evidence indexes may guide navigation but cannot establish this verdict",
                "one canonical owner",
                "consumer points to that owner",
                "distinct consumer-specific decision or consequence",
            ),
        )

    def test_final_candidate_admission_rechecks_only_selected_or_affected_candidates(self) -> None:
        coverage = read(REFS / "service-logic-coverage.md")
        reviewers = read(REFS / "reviewer-perspectives.md")
        assert_all(
            self,
            coverage + reviewers,
            (
                "reapply admission to the operation's retained selected candidates",
                "only the affected candidate closure for a non-baseline operation",
                "Existing file length, AID count, risky code, destructive names, or prior reviewer PASS",
                "risk-shaped code, or an existing AID is not by itself a retention basis",
            ),
        )
    ...
    def test_first_pass_review_routes_meaning_structure_and_source_locators(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "version-5 selection preflight, then narrowed source-evidence check",
                "narrowed source-evidence check",
    ...
    def test_reviewer_corrections_remove_or_generalize_before_precision(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "rerun version-5 selection preflight",
            ),
        )
    ...
    def test_managed_artifact_corrections_reuse_only_exact_approval(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "restorable exact v5 routing contract",
                ...
                "an open invalidation may only expand or become resolved",
                ...
                "exact v5 guarded-path-to-bundle mapping",
                ...
                "preserving every participant's local or other-contract route",
    ...
    def test_bootstrap_state_transitions_into_approved_goal_scope(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "the only owner of operation profile, discovery/accepted scope",
    ...
    def test_global_baseline_is_full_project_only(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "project-wide integration/baseline reviewer independently reconciles the current corpus and PASSes",
    ...
    def test_validation_contract_points_to_plugin_bundled_read_only_tool(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "--phase metrics-final",
                "Final request-bound checks reject open invalidations",
                "stale PASSes",
                "missing, non-string, v1-v4, future, or unknown marker is invalid",
                "current `baseline`/`project-wide` PASS",
                "--expect-atom-key",
                "valid standard YAML",
                "Do not print AID totals in an ordinary PASS",
                "Add `[STRUCTURAL]` after the compatible PASS prefix",
                ...
                "plugin-vendored `markdown-it-py`/`mdurl` runtime",
                "plugin-vendored pure-Python `PyYAML` runtime",
                "third-party notices are retained under `scripts/_vendor/`",
                "does not recover with a second handwritten Markdown scanner",
                "does not recover with a second handwritten Markdown scanner or depend on host site-packages",
            ),
        )

    def test_version_five_uses_event_journal_without_becoming_quality_gate(self) -> None:
        ...
        assert_all(
            ...
            (
                ...
                "python3 <plugin-root>/scripts/record_atomic_docs_event.py",
                "authoritative operation timing/work record",
                "version, sequence, request_id, event_type, payload, previous_hash, event_hash",
                "64 lowercase `0` characters",
                "stageflow.atomic-docs.operation-event.v1\\0",
                "operation-started",
                "span-started",
                "basis_revision,started_at",
                "supplied basis that differs from that state owner",
                "latest finished receipt-bound readiness PASS",
                "current readiness review ID",
                "not retroactively imposed on an older epoch",
                "current-basis development/risk review follows the latest successful pair",
                "dedicated",
                "non-overlapping journal order",
                "span-finished",
                "operation-finished",
                "exact reducer projection",
                "operation wall-clock as the primary observation",
                "completed review/event/rerun counts as secondary workload observations",
                "never an ETA, guarantee, validator threshold",
                "Keep only the root and this latest validation span active",
                "Start final `docs|baseline` validation only after every other span is finished",
                "After FAIL, keep the root active",
                "latest event to be that final validation finish with PASS",
                "cannot downgrade the required phase",
                "--phase metrics-preterminal",
                "--phase metrics-final",
                "Only then retire allowed temporary inputs",
                "active or retired stable `bundle_id`",
                "Risk review never uses `selection-readiness`",
                "current journal's canonical hash chain and exact projection equality",
                "snapshot projected-span progression",
                "does not prove a byte-equivalent retained journal prefix",
                "sequence gaps, bad previous/event hashes",
                "partial rewrite inside the current chain",
                "projection mismatch",
                "entire request with an older internally valid complete chain or valid prefix",
                "not immutable",
                "Python has no portable directory `fsync` on Windows",
                "parent-directory metadata survival across sudden power loss as best-effort",
            ),
        )

    def test_selection_contract_requires_version_five_after_goal_without_migration(self) -> None:
        text = read(SKILL) + refs(
            "docs-generation-flow.md",
            "reviewer-perspectives.md",
            "validation-contract.md",
        ...
        assert_all(
            ...
            (
                ...
                "create or resume only exact `context_selection.version: \"5\"`",
                "Reject a missing, non-string, v1-v4, future, or unknown post-Goal marker",
    ...
    def test_normal_operation_links_risk_before_selection_validation(self) -> None:
        ...
        self.assertLess(
            text.index("route each selected risk trigger"),
