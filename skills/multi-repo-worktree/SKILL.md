...
- **submit**: The development owner may push only its own committed task branches. Write every PR title and all explanatory prose in its body in Korean, while preserving literal code identifiers, paths, commands, URLs, and repository-required fixed template text. Show the exact Korean title and body before approval. Create or update regular non-Draft PRs, then record each pushed full head with a machine-readable `<!-- stageflow-submitted-sha: <full-head-sha> -->` comment and the branch/PR identity in the slot manifest. Release the slot only after every repo's remote handoff and manifest record succeed. A later head change invalidates the handoff until the current head is submitted again; unrelated PR comments, reviews, or status updates do not invalidate an unchanged submitted SHA.
