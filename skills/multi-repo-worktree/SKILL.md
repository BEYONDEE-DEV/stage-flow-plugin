...
create once
  ...
  -> sync deletes the old local PR branch and its exact remote ref
  -> submit ready PR B
  -> repeat
```
...
- `sync` runs only in the development bundle. Fetch updates shared remote-tracking refs, but it never switches or updates the original worktree's HEAD, index, files, or local source branch. After a recorded PR is exactly `MERGED` and rotation preserves its local continuation, `sync` also removes that submitted branch locally and from its recorded remote.
