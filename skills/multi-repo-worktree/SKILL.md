...
- `submit` owns task-related-path Korean commits, same-branch push, Korean ready PR creation or update, exactly proven single-generation merged-PR reconciliation, merged-base incorporation, changed-files proof, and PR-generation recording.
...
- Do not ask for manual manifest editing when exactly one omitted successor PR is provably `MERGED` on the same base/head. Before taking the submit lock, prove from read-only GitHub queries, remote refs, and the local graph that the remote task ref still equals the successor head, the successor head is an ancestor of local `HEAD`, and the successor merge commit is in the remote source history. Revalidate after lock/fetch, reconcile it, reclassify it as `MERGED`, and continue the same submit.
...
- **submit**: Classify every changed repository independently as manifest-proven `NONE`, exact recorded `OPEN`, exact recorded `MERGED`, or a narrowly recoverable omitted successor of an exact recorded `MERGED` PR; supported classifications and generations may differ within one batch. Complete every recovery proof read-only, acquire the slot operation lock, fetch and re-inspect, reconcile only a uniquely and completely proven omitted successor, then reclassify it as `MERGED`. Automatically commit only clearly task-related staged, unstaged, and non-ignored untracked paths with generated Korean messages. `NONE` creates its first ready PR, `OPEN` pushes to the same PR without rewriting its title/body unless the user explicitly requested that edit, and `MERGED` fetches and pins its base, merges it into the same task branch, proves `base...head` and the created PR contain only the next work, then creates the next ready PR. Atomically advance only repositories that reconciled a proven omitted successor or created or exactly adopted a new PR, using each repository's own expected generation. A merged state with no next-work diff is a successful no-op. Always release only the operation lock after a safe terminal result; never release the slot.
...
`CLOSED`, missing or inaccessible recorded PRs, head/base mismatch, unexpected remote divergence, nonlinear or incompletely proven lost manifest history, ambiguous PR adoption, and unknown API state are unsupported. Exactly one omitted successor `MERGED` PR is recoverable only under the full proof contract in `worktree-operations.md`; a second later PR for the same exact base/head is ambiguous even when it is not a descendant. Multiple candidates or any missing SHA, base/head, remote-ref, local-HEAD, merge-commit, or remote-source proof fails closed. Do not reopen, replace, reset, force-push, guess, or bypass this skill with an untracked Git/GitHub flow. A mix of otherwise valid `NONE`, `OPEN`, and `MERGED` repositories is not a blocker.
