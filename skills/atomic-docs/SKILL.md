...
Create and maintain source-based project documentation that gives developers useful implementation context before they inspect or change code. Preserve selected source-established purpose, important rules and contracts, non-obvious constraints, ownership and change-impact context, approved requirements, and material unresolved decisions. Atomic docs guide source inspection; they are not a product-behavior specification, do not replace the source tree, and do not make docs-only reimplementation a quality target. A scope is deep enough when a developer can find the relevant source, understand why the documented area exists, recognize the important constraints and contract touchpoints, and identify likely conflicts. Exact fields, branches, state mechanics, and failure paths may remain in source.
...
- For each newly created operation, record selection contract version 1, classify candidates as `write|merge|drop` after minimal source inspection, and pass selection validation before writing. A legacy active operation without this version continues its recorded unversioned flow and in-scope corrections without migration; do not invoke version-1 validation or add version-1 fields. Admission, evidence, and validator details belong to `service-logic-coverage.md`, `project-documents-and-inventory.md`, and `validation-contract.md`.
...
- Keep reviewer output findings-only. A FAIL chooses `remove`, `generalize`, or `correct_selected_claim`; comparison tables remain conditional risk, integration/baseline, or compliance tools.
...
- General source-context docs do not require a verification target for every observed behavior. Keep a durable verification target or invariant only for an approved implementation-basis change or when it is itself important context. Do not enumerate Cartesian products of inputs, states, failures, actor variants, or execution steps. Derive detailed cases during implementation or review and put repeatable checks in actual tests; do not preserve them in managed docs or routine review artifacts. Add a field list, state table, payload matrix, or failure matrix only when that structure is itself a durable contract or necessary context that prose would make ambiguous.
...
7. For a version-1 selection operation, classify `write|merge|drop`, build the queue from write keys and merge targets, link applicable risk triggers to selected Atom keys/contracts, then run selection validation. Confirm shared/high-fan-out owners before dependents; targeted work checks only local and adjacent owners.
