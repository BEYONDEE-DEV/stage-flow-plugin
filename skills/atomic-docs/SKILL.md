...
- Support `repository` storage in the project and `submodule` storage in a documentation submodule. Never assume a hardcoded `docs/` root.
...
- Before the execution queue, route every selected risk trigger as a local or shared-contract concern, including when one domain has several shards. Close only high-fan-out contracts before dependents; keep local owners provisional. Then route each selected risk trigger and run structural selection validation.
- After Goal handoff, initialize selection contract version 5 and `review_alignment.version: "1"` with receipt-bound closure, binding traces, journal projection, retirement, and dispatch cutoffs. Only a pre-Goal bootstrap request may resume before selection exists. Reject missing, older, future, or marker-free review state and start a new request; never migrate, backfill, or dual-read it. See `shared-contract-readiness.md`.
- Embed the proof receipt owned by `semantic-review-proof.md` in every readiness and semantic PASS. Keep the development reviewer as the persistent primary across bundles, run exactly one blind fresh challenge on each terminal-ready current basis, reuse a qualifying integration/baseline review when possible, and hand off the primary only for the contract's recorded conditions.
...
- Process domain bundles sequentially; one durable domain/shard bundle may contain several atoms. Reuse one writer, one persistent primary development reviewer, and one conditional risk reviewer. After the fourth completed bundle, check primary context saturation before the next dispatch without forcing replacement; use append-only affected-scope handoff only when `semantic-review-proof.md` permits it.
...
- After each writer, run structural preflight. Meaning changes receive development review, structure-only changes scoped validation, and locator-only changes a narrow evidence check. Evidence-only maintenance may replace a writer pair only with unchanged acceptance and current evidence fingerprints; otherwise rerun the writer. See `shared-contract-readiness.md`. Parallel development/risk review shares one bundle attempt; reruns follow actual impact.
...
- Final integration/baseline review reconciles shared acceptance contracts without restarting local discovery. It may reopen material permission, privacy, destructive, or shared-contract omissions as `escaped_prepass_defect`; preserve later findings and separately fail incomplete same-root batching.
...
- Use frontmatter `atom_key` as stable identity. Define meanings with `[AID:...]` and consume them only with `[AID-REF:...]`. Assign AIDs only to important rules/contracts, required changes, active judgments/gaps, or accepted downstream references. Plain observations have none by default. Preserve AIDs; never auto-clean or renumber them. Reject duplicate definitions and dangling references immediately; defer the v5 `created_aids` orphan check until the final request-bound gate so planned downstream consumers may appear later.
...
- Keep `Intent`, `Outcomes`, `Boundaries`, `Rules`, `Current Implementation`, `Planned Changes`, and `Gaps` separate. Each meaning has one owning section; another section may use `[AID-REF:...]` or cite its heading with only the minimum reading context. Distinguish source-established current contracts from approved desired changes and genuinely unresolved decisions under `atom-format-and-judgment.md`; AI authorship alone does not require an inference marker or Gap.
...
- For version-5 removal/rollback, map each guarded snapshot path through its artifact/action `atom_key` to the exact stable queue `bundle_id`; invalidate that bundle's current development/risk PASSes, never another same-domain shard. If an approved-existing source key is absent from the queue, retain only final-gate protection instead of inferring a bundle.
- Append v5 work and `maintenance` events to request-local `operation-events.jsonl`; its canonical hash chain owns history and `operation_metrics` is only the exact projection. Semantic spans keep reviewer verdicts; protocol failures use separate validation spans and still block terminal readiness. Use actual observations only for reporting, never quality.
...
- `references/shared-contract-readiness.md` for the normative version-5 local/shared risk routes, selected binding traces, bounded contract state, pre-writer readiness gate, late discovery, and adaptive FAIL diagnosis.
...
- `references/semantic-review-proof.md` for embedded PASS receipts, persistent-primary identity, blind terminal challenge, and conditional reviewer handoff.
- `references/operation-metrics.md` for the v5 canonical event journal, state projection, terminal checks, and rollback-detection boundary.
...
7. For a new version-5 operation, classify `write|merge|drop`, route risks, create binding traces, and initialize receipt, closure, journal, AID, retirement, and review-alignment state.
8. Require persistent-primary readiness against current acceptance/evidence fingerprints, validate selection, then create the writer against the same acceptance. Report queue counts.
9. Reuse that primary and one writer across sequential selected bundles. Process each through writer, structural preflight, and receipt-bound semantic review; parallel development/risk review only against the same attempt.
...
11. Reapply final admission, integration lint, and reconciliation. A material safety escape reopens readiness and pauses dispatch through correction.
12. Resolve closure, run or reuse exactly one blind fresh challenge for the terminal-ready basis, retain inventory/evidence and receipt reports, then rerun final selection plus request-bound docs/baseline and both terminal journal checks; retire temporary state only after terminal PASS.
