...
- Before a multi-domain queue, confirm shared/high-fan-out owners and order them before dependents. Keep ordinary local owners provisional; do not freeze every aggregate owner.
- New operations use selection contract version 3: validated `write|merge|drop`, closure version 1, and metrics version 1. Older operations continue without migration; details belong to direct references.
...
- Version 3 records operation, bundle, role, validation, and rerun timing under `operation_metrics.version: "1"`; wall-clock is primary workload evidence, never a quality threshold.
...
- `references/operation-metrics.md` for version-3 timing/rerun records, terminal checks, and snapshot progression.
...
7. For a new version-3 selection operation, classify `write|merge|drop`, build the queue from write keys and merge targets, link applicable risk triggers to selected Atom keys/contracts, initialize semantic review closure and operation metrics, then run selection validation. Confirm shared/high-fan-out owners before dependents; targeted work checks only local and adjacent owners.
...
10. In a version-3 or version-2 operation, open semantic review invalidation before a post-PASS meaning or ownership correction and route only affected reruns. Record version-3 rerun timing without changing closure meaning. Older operations use their recorded correction/review flow.
...
12. Resolve semantic closure, run request-bound final docs or baseline validation, close its recorded validation span, then run the uninstrumented preterminal and final metrics checks for version 3.
