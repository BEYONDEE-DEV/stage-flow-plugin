...
- Before a version-4 queue, route every selected risk trigger as a local or shared-contract concern, including when one domain has several shards. Close only high-fan-out permission, shared-identifier, money-entitlement, final-projection, and integration contracts before dependents; keep ordinary local owners provisional.
- New operations use selection contract version 4 with the existing closure/metrics contracts, mandatory empty-first append-only bundle/contract retirement history, and metric-backed dispatch cutoffs. Reuse the persistent development reviewer for a pre-writer readiness PASS and stable bundle-scoped development/risk review; final request-bound and terminal checks revalidate current routing/readiness/dispatch/history. Version 3 and older operations continue without migration. See `shared-contract-readiness.md`.
...
- `references/operation-metrics.md` for version-4/3 timing and rerun records, terminal checks, and snapshot progression.
...
12. Resolve closure, retain inventory/evidence, and for version 4 rerun final selection plus request-bound docs/baseline and both terminal checks against current routing/readiness/dispatch and append-only retirement history; retire temporary state only after terminal PASS. Version 3 keeps its recorded metrics flow.
