...
- Run the domain and final implementation-reconstruction perspectives as fresh docs-only reviewers. Give them managed docs, approved criteria/project context, and required plugin principle files, but not the source tree, operation inventory, or source evidence packet. Needing source or an arbitrary implementation decision is a reconstruction FAIL; source reviewers separately verify factual fidelity against source.
...
- Keep implementation verification conditions lightweight. A confirmed required `Intent`, `Rules`, or `approved_required_change` AID includes its observable verification condition or invariant in the same meaning item; create another AID only for an independently reviewable meaning.
- Do not rely on existing graph edges alone when checking impact. Inspect relevant domain/common context, glossary source-of-truth terms, shared payload/state/storage/permission/integration contracts, and operation-inventory ownership before graph traversal, then record durable findings in existing atoms/graph or unresolved findings in existing plans, gaps, and reviews.
- Only Atomic Impl or an explicit docs/code compliance operation writes an implementation-verification table. Use `## 구현 검증` in `.stageflow/atomic-docs/requests/<request-id>/post-write-review.md`, record docs and implementation basis once, and list only changed in-scope required AIDs with implementation evidence, validation evidence, and verdict or gap. Do not put this table in managed docs, project inventory, `work-state.json`, or a new trace file.
...
7. Process each domain bundle through one writer and four draft reviewers, keeping reconstruction docs-only and source fidelity source-backed, until all four PASS before starting the next bundle.
