...
Create and maintain source-based project documentation that preserves source-established current contracts, approved requirements, and important unresolved decisions developers must not have to rediscover or invent. Atomic docs guide source inspection; they do not replace the source tree, mirror every implementation branch, or default to a vulnerability audit. A scope is documented deeply enough when product behavior is unambiguous, verification can be derived, and affected contracts or conflicts are visible while internal technical choices remain free.
...
- Measure document depth by decision completeness, not source coverage volume, atom count, line count, or whether source can be discarded. Preserve current contracts, approved requirements, and material unresolved decisions when applicable; do not turn ordinary docs generation into a forensic defect, attack-scenario, or exhaustive failure inventory.
...
- Create a `Gaps` item only when a mismatch, uncertainty, or missing evidence prevents a stronger implementation or review judgment. AI authorship or the general absence of user approval does not make a source-established current contract a gap. A missing test, possible runtime exception, or isolated source observation is not a gap by itself. Combine observations only when they share one judgment label, one unresolved decision, and a compatible resolution; keep different labels, independently resolvable behavior, high-risk contracts, adverse branches, and verification outcomes separate.
