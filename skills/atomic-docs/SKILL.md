...
- After Goal handoff, initialize exact selection contract version 5 with mandatory receipt-bound closure, binding traces, event-journal projection, empty-first retirement history, and dispatch cutoffs. Only a pre-Goal bootstrap request may resume before selection exists. Reject every post-Goal missing, non-string, older, or future selection marker and start a new version-5 request under the existing approval/Goal gates; never resume, migrate, backfill, or dual-read it. See `shared-contract-readiness.md`.
...
- Append v5 work events to request-local `operation-events.jsonl`; treat its canonical hash chain as the authoritative timing/work history and keep `work-state.json.operation_metrics` only as the exact reducer projection. Record starts and finishes as separate events. Use actual elapsed/review/rerun observations only for reporting, never readiness or semantic quality.
...
7. For a new version-5 operation, classify `write|merge|drop`, route each selected risk trigger, create its required contract/resource binding trace, and initialize receipt, closure, journal, AID-reference, and retirement state.
8. Create the persistent primary development reviewer, require a receipt-bound selection-readiness PASS, run structural selection validation, then create the writer. Record the compact write plan and report domain, bundle, risk, shared-owner, and final-review counts.
