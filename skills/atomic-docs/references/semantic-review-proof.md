...
Also bind every readiness/semantic review span through `review_alignment.bindings`. All applicable participants use the same accepted-meaning fingerprint(s); readiness records its launch-time evidence, while later semantic reviewers name each evidence revision/fingerprint they directly inspect. Journal `span-started` stores the canonical binding hash so rewriting `work-state.json` cannot rewrite an older writer/reviewer basis. Never rewrite the readiness receipt after evidence maintenance: preserve its launch-time manifest/report, and require the current evidence-only overlay receipt to hash the updated raw inventory/evidence inputs. The receipt continues to prove exact docs/source inputs and report bytes, while review alignment proves that writer and reviewers judged the same accepted contract. Validate receipt output independently; do not feed it back into its own evidence fingerprint, put mutable evidence digests into the acceptance fingerprint, or treat a changed receipt as changed documented meaning.
...
Run at most one challenge attempt for a basis revision and only after that basis is terminal-ready: every required full development/risk review follows its successful bundle/writer attempt, every evidence-only development review follows its accepted same-basis maintenance attempt, all required reviews are current, invalidations are resolved, dispatch is ready, and final candidate admission is complete. In journal order, the challenge follows every current bundle review and the last bundle/writer or maintenance correction. Final closure requires exactly one attempt at the current closure basis and exact `PASS`.
