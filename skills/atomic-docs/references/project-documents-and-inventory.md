...
A service logic inventory is operation-local by default. Keep it under `.stageflow/atomic-docs/requests/<request-id>/inventory.md` or `work-state.json` from domain proposal through writer/reviewer cycles and, for version 4, through final request-bound selection/docs/baseline plus both terminal checks. Retain `<doc-root>/project/service-logic-inventory.md` only when the accepted scope explicitly asks for a final implementation-context index.
...
One root unresolved decision has one canonical owner in the narrowest shared project/context or domain Atom that can resolve it. A consumer points to that owner by `atom_key` and AID or owning heading; it creates a local Gap only for a distinct consumer-specific decision or consequence. In version 4, 3, or 2, when glossary, inventory, context, or an Atom changes that owner after a review PASS, treat every changed owner/consumer projection as an affected artifact under `semantic-review-closure.md`. Legacy operations route the same correction through their recorded review flow without adding closure state.
...
For multi-domain or `initial-baseline` work, run one prepass over selected shared/high-impact context candidates before the first writer. A targeted single-domain operation checks only the target owner and adjacent contracts; it does not perform project-wide ownership discovery. New version-4 operations apply the bounded kinds, local/shared disposition, owner/consumer evidence closure, and readiness gate from `shared-contract-readiness.md`. Earlier operations follow their recorded ownership prepass.
...
After these anchors are known, finalize `write|merge|drop` and update planned Atom keys and queue. Version 4 requires the same persistent development reviewer's readiness PASS under `shared-contract-readiness.md`, then the selection preflight from `validation-contract.md`, before the first writer; earlier versions follow their recorded preflight order. A newly discovered candidate repeats only the affected admission/readiness step; it does not expand `evidence.md` into behavior closure.
...
After accepted atom docs are written and reviewed, version 4 keeps operation-local inventory/evidence until final selection, request-bound docs/baseline, `metrics-preterminal`, and `metrics-final` all PASS. Only then delete or ignore them: delete or ignore the operation-local inventory/evidence unless the user explicitly approved retaining a synced project context index; earlier versions keep their recorded cleanup point.
