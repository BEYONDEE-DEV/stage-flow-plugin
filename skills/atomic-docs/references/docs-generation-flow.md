...
Reviewer reports must include a reviewer answer sheet. Answer whether the same functional behavior can be implemented from docs alone, what fields/branches/validation/state/failure details still require source, whether any source identifiers appear without natural-language behavior, and whether every source surface/aggregate has an `atom_key`/AID, coverage gap, `out_of_scope`, or not-applicable disposition. Include a claim audit table in the shape `AID claim -> checked entry path -> null/blank/default/fallback/exception branch -> source evidence` for judgment-bearing claims.
...
Reviewer subagents must fail any retained or operation-local inventory row that lacks row-level closure to an owning `atom_key`/AID, coverage gap, `out_of_scope`, or not-applicable reason, or that omits required conditions/branches, validation/permission, state transition, persistence, integration/event, error/recovery, and source evidence fields for a meaningful behavior.
...
The post-write review must also re-check source discovery closure and row-level inventory closure. Any source-observed behavior with no `atom_key`/AID, coverage gap, `out_of_scope`, or not-applicable disposition remains provisional and prevents source-baseline metadata updates and judgment-ready or implementation-reconstruction-ready reporting.
