...
When atomization criteria are needed, do not keep reviewed atomization perspectives only in chat or only in the change plan. For Korean managed docs, record them in the criteria document's `Atom화 관점` section. The authoritative list of required visible Korean headings and per-perspective subfields lives in `atomization-criteria-contract.md`; use that reference as the single source and do not introduce heading name variants here. After the docs root is confirmed, make the first atomic-docs write action a limited draft creation or update of `<doc-root>/project/atomization-criteria.md` as the criteria proposal.
...
- full discovery candidates, approved domain/category boundaries, accepted-scope rules, and atom candidate map entries are mixed together so the reader cannot tell what is merely discovered, what is a durable boundary, what is a behavior-level atom candidate, and what a later operation may accept
- durable domain approval status is used to encode operation-local write scope
- the approved criteria contains the live bundle queue, active domain bundle, writer/reviewer PASS/FAIL log, temporary inventory status, post-write gate result, baseline-ready decision, or other current-run state that belongs under `.stageflow/atomic-docs/requests/<request-id>/`
...
- a concise Korean summary of what was written, including docs root and accepted-scope rules, domain partitioning criteria, project-native feature candidates, capability/common promotion proposals, candidate or approved domain/category boundary map, atom candidate map, atomization perspectives, shared writer/reviewer quality criteria, service logic coverage requirements, judgment policy usage, and open blockers as separate items
