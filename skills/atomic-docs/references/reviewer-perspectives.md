...
- current-contract basis: source-established `Outcomes`, `Boundaries`, and `Rules` are supported by reachable behavior and the applicable global baseline or operation-local `source_commit_observed` without a blanket inference Gap
...
Review only triggered concerns that can alter an accepted implementation, verification, or change-impact decision. Check adverse branches, permission or security boundaries, irreversible effects, money/entitlement changes, transaction/idempotency, retries/recovery, external contracts, sensitive data, and shared cross-domain contracts as applicable. Require the durable risky contract and concise verification target, then derive concrete test combinations transiently during implementation/review instead of preserving their full cross-product in the atom or routine review report. For an external contract, require authoritative local or user-approved provider evidence when the remote guarantee is necessary; otherwise require `confirmation_needed` only when the missing guarantee blocks the judgment.
