...
Run one additional independent reviewer when operation state records at least one `service-logic-coverage.md` risk trigger tied to a selected candidate/Atom contract or an approved implementation-basis change. The development-quality reviewer independently checks trigger completeness and its local/shared route. If it FAILs on a missing candidate-linked trigger, correct state, rerun version-4 selection preflight, and rerun both development and applicable risk reviews against the same corrected revision; never reuse the old FAIL as PASS. Code containing a possible risk surface and a domain-level risk category are not by themselves requests for a retained risk audit. Record the candidate ID, planned `atom_key`, trigger, selected contract basis, and route in operation state before review and reuse the same risk reviewer across triggered bundles. Several candidate contracts may reference the same Atom key and remain separate trigger entries.
...
| Source locator/evidence index correction with unchanged documented claim | version-4 selection preflight, then narrowed source-evidence check by the development reviewer |
...
| Risk-trigger addition/removal or candidate/Atom route | version-4 selection preflight, then development-quality and applicable risk/contract reviewers on the corrected revision |
...
- When `remove` or `merge` changes candidate disposition, expected Atom keys, or risk references, rerun version-4 selection preflight before the writer or semantic rerun.
