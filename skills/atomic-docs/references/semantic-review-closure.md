...
This reference is the normative owner of forward-only semantic review invalidation state, its transition order, and the request-bound final closure gate. `reviewer-perspectives.md` decides which semantic reviews are required and whether their findings PASS. `semantic-review-proof.md` owns each PASS receipt and the terminal blind challenge. `docs-generation-flow.md` invokes transitions around writes, removals, merges, and owner changes. `validation-contract.md` defines only deterministic validation of recorded state.

## Forward-Only State

Every post-Goal operation uses exact `context_selection.version: "5"` and adds `semantic_review_closure` to `work-state.json` before candidate writing. It also uses the authoritative event journal/projection from `operation-metrics.md`, readiness state from `shared-contract-readiness.md`, and proof state from `semantic-review-proof.md`.
...
Open an invalidation before changing a meaning that already contributed to a PASS. A version-5 late discovery after readiness but before any affected bundle semantic PASS has nothing to invalidate and records `semantic_invalidation_ids: []`; once an affected PASS exists, invalidation is mandatory before correction. Triggers are `candidate-disposition`, `atom-merge`, `ownership`, `glossary-source`, `shared-decision-owner`, `graph-relationship`, or `documented-meaning`.

An accepted evidence-only maintenance attempt does not change documented meaning and therefore does not open a meaning invalidation merely to refresh locator/evidence/receipt binding. Keep the current full semantic PASS as meaning owner and preserve readiness proof; the latest narrow evidence-only development PASS becomes the current evidence overlay, while an older overlay may become `superseded` without semantic invalidation. The attempt must preserve the canonical acceptance fingerprint and route through its same-basis maintenance span plus overlay PASS. A fingerprint difference or non-allowed field change is a meaning correction and follows the normal invalidation/writer path.
...
When one correction affects an earlier shared owner, include that owner bundle ID and only consumer bundle IDs whose recorded basis changed. For version-5 late discovery, `shared-contract-readiness.md` owns the discovery/pause record and this closure owns only post-PASS staleness: use no invalidation before an affected semantic PASS, otherwise use the applicable ownership/shared-decision trigger and leave unrelated bundle PASSes current. Retired bundle/contract IDs keep historical reviews, invalidations, diagnostics, and discoveries interpretable but never become active routes. When a later unrelated correction increments the operation revision, an already resolved invalidation remains valid. If a later change affects its artifacts, owner/consumer basis, receipt inputs, binding trace, or required review result, open a new invalidation rather than rewriting the resolved record.

A final-review `escaped_prepass_defect` is not hidden inside a local documented-meaning correction. It first stales readiness and pauses dispatch under `shared-contract-readiness.md`, then opens the affected semantic invalidation(s) needed for the named root fan-out. Keep repeated material findings even when the first fan-out was incomplete; the separate batching protocol FAIL blocks terminal progress until the expanded correction and readiness rerun resolve.
...
- run or reuse the exact current-basis blind terminal challenge under `semantic-review-proof.md` and require its PASS
- retain request inventory/evidence and receipt reports, then run the validator with the request ID for final selection plus request-bound docs/baseline and terminal checks so version-5 routing, binding traces, journal projection, immutable retirement history, receipt-bound readiness, ready dispatch, and this closure are checked together

The final reviewer and blind challenge independently compare current managed docs and raw source inputs under their owning contracts. This state and its receipts prove only that declared current-basis inputs/reviews were recorded and no known invalidation was left open; they do not prove that owner, Gap, candidate, binding trace, or natural-language judgments are semantically correct.
