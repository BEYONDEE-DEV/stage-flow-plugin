...
The required v5 routing, shared-contract, binding trace, stable bundle, readiness, late-discovery, diagnostic, dispatch, closure, receipt/challenge/handoff, journal projection, `created_aids`, retirement, and review-alignment owners come from their routed references. Initialize retirement as exact `{"version":"1","retired_bundles":[],"retired_contracts":[]}`, handoffs as exact `{"version":"1","history":[]}`, challenge as exact `{"version":"1","attempts":[]}`, `created_aids: []`, and exact `review_alignment.version: "1"` with acceptance contracts, bindings, maintenance attempts, and escaped-prepass defects. The review-alignment marker is itself a hard-cut sub-contract: reject marker-free older v5 state and do not migrate or dual-read it.
...
Derive every active bundle's accepted-meaning projection separately from mutable evidence. Derive evidence from candidate-scoped input sections and bundle-owned locator/content bindings, not whole-file hashes, review result receipts, or review IDs. Verify its acceptance fingerprint, current evidence revision/fingerprint, immutable journal-bound writer/reviewer bindings, and exact bundle coverage. An evidence-only review must follow one completed same-basis maintenance span whose journal start matches its prior contract, preserve its before/after acceptance fingerprint, advance the evidence revision exactly once to the current fingerprint, touch only allowed evidence/receipt/protocol fields, and own exactly one linked `review_mode: evidence-only` development PASS that hashes current raw inventory/evidence. Keep one current full development PASS as meaning owner and one current evidence overlay; do not rewrite readiness/full-review bindings to the new evidence. Any accepted-meaning difference requires the normal writer path.

Keep semantic and protocol outcomes separate. A semantic PASS record requires a matching review metric PASS; the event recorder rejects rewriting that span to FAIL because chronology, receipt, order, or projection validation failed. Such failures use a separate `validation` span, never satisfy or create a semantic FAIL diagnostic, and block the terminal-ready challenge until a later same-scope PASS as well as blocking final completion. Validate reciprocal escaped-prepass diagnostic/defect ownership, exact root fan-out, root-scoped new-evidence identity, stale readiness, matching dispatch pause, repeated-root batching protocol FAIL, and corrected readiness/invalidation resolution. These deterministic checks prove recorded routing and hashes, not that the natural-language contract is complete.
