...
- `TARGETED_SYNC_REQUIRED`, `FULL_CONSISTENCY_REQUIRED`, and `SNAPSHOT_CURRENT_REQUIRED` responses run definition validation on Stop. If the gate is still incomplete, Stop blocks only completion, approval, sync-complete, or next-stage claims; status-only responses may end, and the next `UserPromptSubmit`/`PreToolUse` continues to enforce the same gate.
