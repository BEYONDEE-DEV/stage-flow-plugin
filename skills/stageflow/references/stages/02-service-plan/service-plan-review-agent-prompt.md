...
- Check that `## Pending Clarifications` records any unanswered active service clarification batch with concrete service questions, at least two proposal options, a recommended option, the `구현 계획으로 넘어가기` transition option, rationale, and pending/awaiting status.
- Check that `## Clarification History` records answered service-behavior questions, at least two proposal options, the `구현 계획으로 넘어가기` transition option, user responses, and reflected artifact areas.
...
- Mark `FAIL` when the same pending service question is recreated as a new round instead of preserved, or when a user follow-up about an option is answered without restating the still-pending questions and options.
...
- If a user selects a service proposal option, verify the answer is reflected and followed by another service clarification round or pending batch. If a user selects `구현 계획으로 넘어가기`, verify the transition choice is recorded before approval.
