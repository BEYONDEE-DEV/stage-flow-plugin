...
- Required clarification hot-path store: `01-definition/definition-store/`
- Conditional question scope transition review: `01-definition/question-scope-transition-review.md` when pending questions move from `큰방향` to `주요결정` or from `주요결정` to `세부확인`
...
- Check that `definition-store/` exists whenever clarification is active, `working-set.json.active_pending_questions` is the canonical pending batch during active clarification, `decision-ledger.jsonl` records answered decisions, `trace-index.json` covers affected IDs, and `sync-state.json.current_gate`/`decision_sync` show the required targeted/full/snapshot sync before lower-scope questions, review, approval, or implementation planning.
- When current pending questions are `주요결정`, confirm `01-definition/question-scope-transition-review.md` has a current-fingerprint `큰방향 -> 주요결정` PASS row from a question scope transition review subagent. When current pending questions are `세부확인`, confirm both `큰방향 -> 주요결정` and `주요결정 -> 세부확인` current-fingerprint PASS rows exist.
...
- Check that `구현 계획으로 넘어가기` and equivalent stop signals are recorded only as user stop signals in `Clarification History.User Transition Signal`, not as pending question options or `Transition Option` values.
...
- Mark `FAIL` when the latest user answer has no following active pending clarification batch in `definition-store/working-set.json` and no recognized sync gate (`targeted-sync-required`, `full-consistency-required`, or `snapshot-current`), when the batch has more than five active questions, when active pending rows mix multiple question scopes, when a question has invalid question scope or fewer than two labeled options, when definition-store active pending IDs/questions/traces are stale, when medium/high-risk decisions are not synced at their required gate, when store-only turns read or write full `definition.md`, when the agent moves from 큰방향 to 주요결정/세부확인 without recorded basis and current PASS evidence, when it moves from 주요결정 to 세부확인 without current PASS evidence, or when there is no explicit user stop signal.
...
- Check `## Flow Extraction Checklist` in the writing rules, then verify that `## Approved Flow Inventory` records every major approved user, system, policy, integration, failure/empty-state, and boundary flow as a `DFLOW-*` row with source IDs, trigger, actor/consumer, target outcome, state/data responsibility, failure or empty behavior, and valid boundary status.
- Confirm every concrete `REQ-*` and `SP-*` appears in at least one `Approved Flow Inventory.Source IDs` cell. When an approved `REQ-*` or `SP-*` clearly implies a missing `DFLOW-*`, require artifact repair of the flow inventory rather than a transition-risk question.
- Mark `FAIL` when a major flow is only implied by prose, when a `REQ-*` or `SP-*` is uncovered by `DFLOW-*`, when a `DFLOW-*` row lacks `DEC-*`, `REQ-*`, `SP-*`, or `INTENT-*` source support, or when an out-of-scope/external-boundary flow that can affect implementation planning is missing from the inventory.
