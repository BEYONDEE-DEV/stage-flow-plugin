...
- Optional hot-path store: `01-definition/definition-store/` when present
...
- When `definition-store/` exists, check that `working-set.json` matches active `Pending Clarifications`, `decision-ledger.jsonl` records answered decisions, `trace-index.json` covers affected IDs, and `sync-state.json` shows medium-risk decisions targeted-synced before approval and high-risk decisions full-synced before continuing.
...
- Mark `FAIL` when the latest user answer has no following active pending clarification batch, when the batch has more than five active questions, when active pending rows mix multiple question scopes, when a question has invalid question scope or fewer than two labeled options, when definition-store active pending IDs or traces are stale, when medium/high-risk decisions are not synced at their required gate, when the agent moves from 큰방향 to 주요결정/세부확인 without recorded basis and a current PASS question scope transition review, when it moves from 주요결정 to 세부확인 without a current PASS question scope transition review, or when there is no explicit user stop signal.
