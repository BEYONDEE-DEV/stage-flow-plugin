...
After project inspection, assume there is still some ambiguity to clarify. Do not first decide whether ambiguity exists; behave as if it does, find the next useful definition question, and ask it. The next question should refine user intent, scope, success criteria, constraints, normal behavior, user/system flow, state and policy, integration responsibility, boundaries, regression prevention, or failure recovery.

After every user answer, reflect the answer into the appropriate definition rows, then create the next concrete clarification question. The agent must not close definition by judging the request or behavior model `clear enough`, `충분함`, or complete on its own authority. The loop ends only when the user explicitly gives a stop signal such as `구현 계획으로 넘어가기`, `질문 그만`, `충분해`, `진행`, `승인`, `proceed`, or `go ahead`.

Record unanswered active questions in `## Pending Clarifications`; before a user stop signal exists, this table must contain at least one concrete active question with at least two meaningful proposal options, a recommended option, why the answer matters, and Status `pending` or `awaiting`. `구현 계획으로 넘어가기` is not a proposal option and must not be included inside the `Options` cell; it is only a user-authored stop signal recorded in `## Clarification History`. After presenting a pending clarification batch, complete the Codex goal with `Goal completion reason: awaiting user clarification`, then stop and wait for the user. Do not run review, approval, next-stage work, or mark the goal blocked merely because the user has not answered yet.
...
| DEF-RULE-007 | Keep asking definition questions until the user explicitly stops. | `## Pending Clarifications` and `## Clarification History` show concrete questions, at least two proposal options, recommended options, user responses, reflected areas, and any explicit user stop signal. | Confirm the agent assumes more ambiguity remains, stops while pending, restates still-pending choices after follow-ups, excludes stop signals from question options, and does not close definition without a user stop signal. | Pending clarifications are incomplete, duplicated, hidden after follow-up, include a stop signal as an option, or definition closes because the agent judged it clear enough without a user stop signal. |
