...
After every user answer, record the answer into `01-definition/definition-store/` when that store exists, or reflect it directly into the appropriate `definition.md` rows in legacy mode. Then create or maintain the next concrete clarification batch with 1-5 active questions. The active batch must use one `Question Scope` at a time; do not mix higher-scope and lower-scope rows in the same pending batch. The agent must not close definition by judging the request or behavior model `clear enough`, `충분함`, or complete on its own authority. The loop ends only when the user explicitly gives a stop signal such as `구현 계획으로 넘어가기`, `질문 그만`, `충분해`, `진행`, `승인`, `proceed`, or `go ahead`.
...
`01-definition/definition-store/` is optional and backward compatible. Use it for large definition loops where rewriting the full `definition.md` after every answer would slow the user-facing question flow. `definition.md` remains the approval-ready snapshot, but active clarification may use the store as the hot-path source of truth until snapshot sync is complete.

Required files when the store exists:

- `working-set.json`: records `active_pending_ids`, `current_scope`, latest answer summary, next question candidates, and `risk_level`.
...
- `sync-state.json`: records current `definition.md` snapshot fingerprint and `decision_sync` status for each decision.

Helper subagents may write `impact-candidates.json` and `targeted-sync-plan.json` under `definition-store/` while waiting for the user, but those files are candidates only. The main agent owns risk classification, durable ledger entries, trace updates, snapshot sync, and the next visible pending batch.

Risk levels:

- `low`: copy, label, or minor acceptance detail. Record in the store and keep the question hot path moving.
- `medium`: service policy, service-level data responsibility, exposed data, failure/recovery, or regression semantics. Targeted sync must update affected definition rows before review or approval.
- `high`: reverses a prior decision, changes ownership/scope, or changes auth, payment, privacy, security, or integration responsibility. Full consistency must be complete before continuing to lower-scope questions, review, or approval.

Full consistency is required before `큰방향 -> 주요결정`, before `주요결정 -> 세부확인`, after user stop signals, before definition review/approval, and immediately after high-risk answers.
...
| DEF-RULE-007 | Keep asking definition-level questions breadth-first until the user explicitly stops, and defer implementation-plan-only decisions. | `## Pending Clarifications` and `## Clarification History` show 1-5 concrete definition-level questions, valid `Question Scope`, at least two explicit labeled proposal options, recommended options, user responses, reflected areas, contextual `Why This Matters` explanations, any explicit user stop signal, `01-definition/question-scope-transition-review.md` PASS evidence before lower-scope pending questions, and when `definition-store/` exists, store rows for active pending IDs, decisions, traces, and sync state. | Confirm the agent classifies question stage before asking, asks only definition-level questions, defers module/file/test command/architecture/work item decisions to implementation-plan, starts with 큰방향 questions, explains the practical meaning of the scope label, uses one active question scope per batch, records answers in the store or legacy artifact, moves deeper only with recorded basis plus a passing question scope transition review subagent, restates still-pending choices after follow-ups with context and labeled options, excludes stop signals from question options, and does not close definition without a user stop signal. | Pending clarifications are incomplete, duplicated, hidden after follow-up, context-free, over five active questions, mixed across scopes in one active batch, invalid question scope, missing labeled options, include a stop signal as an option, ask implementation-plan-only decisions, use unexplained internal shorthand, skip breadth-first progression, move to lower-scope questions without a current PASS transition review, have stale or unsynced definition-store evidence, or definition closes because the agent judged it clear enough without a user stop signal. |
