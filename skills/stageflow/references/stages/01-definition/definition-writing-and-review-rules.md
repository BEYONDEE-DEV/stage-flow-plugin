...
| DEF-RULE-007 | Keep asking definition questions breadth-first until the user explicitly stops. | `## Pending Clarifications` and `## Clarification History` show 1-5 concrete questions, valid `Question Depth`, at least two explicit labeled proposal options, recommended options, user responses, reflected areas, and any explicit user stop signal. | Confirm the agent assumes ambiguities remain, starts with broad questions, moves deeper only with recorded basis, restates still-pending choices after follow-ups, excludes stop signals from question options, and does not close definition without a user stop signal. | Pending clarifications are incomplete, duplicated, hidden after follow-up, over five active questions, invalid depth, missing labeled options, include a stop signal as an option, skip breadth-first progression, or definition closes because the agent judged it clear enough without a user stop signal. |
...
| DEF-RULE-016 | Run and confirm transition risk before definition approval after a user stop signal. | `01-definition/transition-risk-goal.md` records the transition-risk goal receipt and `01-definition/transition-risk.md` records generated risk cases, user confirmations, dispositions, and reflected definition updates or exclusions. | Confirm every risk case is user-confirmed, dispositions are allowed, `ask-follow-up` reopens pending clarification, and `apply-to-definition` evidence appears in the relevant definition sections before approval. | A stop signal goes directly to review/approval, transition-risk files are missing or stale, a risk case lacks user confirmation or disposition, a follow-up risk has no active pending clarification, or an applied risk is not reflected in the definition. |
