...
| --- | --- | --- | --- | --- |
| IP-FLOW-001 | Map approved definition flows. | `## Implementation Flow Model` records the `DFLOW-*` source, definition source IDs, trigger, target outcome, primary work items, flow status, and status rationale for each approved user/system/policy/integration flow. | Every approved `DFLOW-*` row from definition is mapped without omission or invented flow identity. | An approved `DFLOW-*` is absent from the plan, or the plan references an unknown definition flow. |
...
| IP-FLOW-007 | Unresolved gaps must block approval or cite definition scope. | Non-complete flows use `return-to-definition`, `out-of-scope-by-definition`, or `external-boundary-by-definition`, cite the definition basis, and explain the status rationale. | Unresolved decisions are returned to definition before approval, true out-of-scope flows match approved exclusions, and external-boundary flows record the observable repo boundary plus consumer responsibility. | A gap is hidden as complete, `return-to-definition` appears in an approval-ready plan, an external boundary is marked complete, or a flow is marked non-applicable without source support. |
...
Allowed `Flow Status` values are `complete`, `return-to-definition`, `out-of-scope-by-definition`, and `external-boundary-by-definition`.

`return-to-definition` is not an approval-ready status. Use it to expose a missing or conflicting definition decision, then return to definition and resolve that gap before implementation-plan review can PASS. `out-of-scope-by-definition` is approval-ready only when the row cites the approved `DFLOW-*` and source IDs that make the exclusion explicit. `external-boundary-by-definition` is approval-ready only when the approved definition marks that `DFLOW-*` as an external boundary; its `Status Rationale` must record the observable boundary this repo provides and the consumer responsibility outside this repo. Do not mark such a flow `complete`.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| FLOW-001 | DFLOW-001 | REQ-001, SP-001 | 승인된 flow를 시작하는 사용자 또는 시스템 진입점. | 사용자가 보거나 시스템 consumer가 확인하는 완료 결과. | WORK-001 | complete | Definition flow is in scope and has no unresolved service decision. |
...
| IP-RULE-003 | Define concrete work items with technical design and evidence. | `## Work Items` table has `ID`, `Implementation Unit`, `Technical Design`, and `Completion Evidence` columns; each work item references its related `FLOW-*`, and each complete flow lists existing `WORK-*` primary work items. | Confirm each work item is small enough to verify independently, detailed enough to implement directly, connected to a complete flow, and not referenced through unknown IDs. | Work items are vague, bundled, file-list-only, lack technical design/evidence, are not tied to a flow, or are referenced by flow/coverage/fidelity rows that do not exist. |
| IP-RULE-004 | Trace approved service policy rules from the definition to technical work and flows. | `## Coverage Matrix` table has `Service Rule ID`, `Work Item ID`, `Change Area`, `Validation Evidence`, and `Risk/Constraint` columns; `## Implementation Flow Model`, `## Flow Completeness Matrix`, and `## Definition Fidelity Matrix` link work items back to `DFLOW-*` and definition sources, including `external-boundary-by-definition` mappings when definition assigns completion outside this repo. | Confirm every service policy rule and approved definition flow is covered by implementation work, validation evidence, flow evidence, fidelity evidence, or source-supported boundary rationale. | A service policy rule or approved definition flow is not mapped to technical work, flow completion, validation, definition fidelity evidence, or approved boundary rationale. |
...
An implementation plan is ready for implementation only when another agent can execute every complete flow without deciding architecture, interfaces, module responsibilities, handoffs, state/data transitions, edge-case behavior, completion evidence, or validation strategy, and every non-complete flow is either returned to definition or closed by approved out-of-scope/external-boundary rationale.
