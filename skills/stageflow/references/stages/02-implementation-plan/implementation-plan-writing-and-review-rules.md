...
| IP-FLOW-007 | Unresolved gaps must block approval or cite definition scope. | Non-complete flows use `return-to-definition` or `out-of-scope-by-definition`, cite the definition basis, and explain the status rationale. | Unresolved decisions are returned to definition before approval, while true out-of-scope or consumer-boundary flows match approved definition sources. | A gap is hidden as complete, `return-to-definition` appears in an approval-ready plan, or a flow is marked non-applicable without source support. |
...
| IP-RULE-004 | Trace approved service policy rules from the definition to technical work and flows. | `## Coverage Matrix` table has `Service Rule ID`, `Work Item ID`, `Change Area`, `Validation Evidence`, and `Risk/Constraint` columns; `## Implementation Flow Model`, `## Flow Completeness Matrix`, and `## Definition Fidelity Matrix` link work items back to `DFLOW-*` and definition sources. | Confirm every service policy rule and approved definition flow is covered by implementation work, validation evidence, flow evidence, and fidelity evidence. | A service policy rule or approved definition flow is not mapped to technical work, flow completion, validation, or definition fidelity evidence. |
