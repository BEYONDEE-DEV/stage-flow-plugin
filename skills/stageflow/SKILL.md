...
- `Stop` blocks unsafe completion or advancement claims, but it does not classify whether the user answered, asked a follow-up, or stopped clarification.
- During `AWAITING_USER`, the skill must read `active_pending_questions`, revise `definition-store/`, set the next gate, and avoid `definition.md` on store-only paths; follow-up turns must restate all pending labeled choices.
