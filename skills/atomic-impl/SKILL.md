...
- Before initial implementation-basis docs, final docs, or compliance work reuses a linked Atomic Docs request, accept only pre-Goal bootstrap state with no `context_selection` or post-Goal exact `context_selection.version: "4"`. Do not mutate or migrate a linked v1-v3/unversioned selection; create a new version-4 Atomic Docs request under the existing approval/Goal gates.
...
- Final docs/code compliance must FAIL when any changed in-scope required AID lacks implementation evidence, validation evidence, or a verdict/gap row.
